...
const int paralelos = 8;			// nb -> paralelos //Resolución de la esfera
...
void sphere_draw()
{
	...
	glDrawElements(GL_LINE_LOOP, sizeof(sphere_ix) / sizeof(GLuint), GL_UNSIGNED_INT, 0);    // indices (choose just one line not both !!!) //LINE_LOOP para dibujar la esfera
	//glDrawElements(GL_TRIANGLES, sizeof(sphere_ix) / sizeof(GLuint), GL_UNSIGNED_INT, 0); //Otra manera de dibujar la esfera
	glBindVertexArray(0);
}
...
void animate(void)
{
	sol += 0.5f; //Para que gire solo, no depende de una tecla
	mercurio += 0.9f;
	venus += 0.8;
	tierra += 0.7;
	marte += 0.6f,
...
void display(void)
{
	...
	sphere_draw();	//Sun
	model = glm::mat4(1.0f); //Reseteamos la matriz para regresar al origen
	...
	projectionShader.setMat4("model", model);
	projectionShader.setVec3("aColor", glm::vec3(0.4f, 0.7f, 0.3f));
	sphere_draw();
	model = glm::mat4(1.0f);
	...
	projectionShader.setMat4("model", model);
	projectionShader.setVec3("aColor", glm::vec3(0.7f, 0.5f, 0.0f));
	sphere_draw();
	model = glm::mat4(1.0f);
	...
	projectionShader.setMat4("model", model);
	projectionShader.setVec3("aColor", glm::vec3(0.3f, 0.1f, 1.0f));
	sphere_draw();
	...
	projectionShader.setMat4("model", model);
	projectionShader.setVec3("aColor", glm::vec3(0.5f, 0.0f, 1.0f));
	sphere_draw();
